...
if(isset($_GET['cid'])){
  ...
  foreach($result as $row){
    ...
    //赛事状态
    if($now>$end_time){
      $view_contest[$i][2] = $MSG_Ended;
      $view_contest[$i][3] = "比赛已结束";
    }else if ($now<$start_time){
  	  $view_contest[$i][2] = $MSG_Start;
      $view_contest[$i][3] = $MSG_TotalTime.formatTimeLength($length);
    }else{
  	  $view_contest[$i][2] = $MSG_Running;
      $view_contest[$i][3] = $MSG_LeftTime.formatTimeLength($left);
    }
    //参赛权限
    ...
    //开始结束日期
    $view_contest[$i][7] =  substr($row['start_time'],2,14);
    $view_contest[$i][8] =  substr($row['end_time'],2,14);
    $i++;
  }
}
