...
$result=pdo_query( $sql,$id);
 $row=$result[0];
$acuser=intval($row[0]);
$view_problem[2][0]="通过人数";
$view_problem[2][1]=$row[0];


$jresult2=Array($MSG_PD,$MSG_PR,$MSG_CI,$MSG_RJ,"通过数",$MSG_PE,$MSG_WA,$MSG_TLE,$MSG_MLE,$MSG_OLE,$MSG_RE,$MSG_CE,$MSG_CO,$MSG_TR);
        $i=3;
        $sql="SELECT result,count(1) FROM solution WHERE problem_id=? AND result>=4 group by result order by result";
        $result=pdo_query( $sql, $id);
        foreach($result as $row){

                $view_problem[$i][0] =$jresult2[$row[0]];
                $view_problem[$i][2] =$row[1];
                $view_problem[$i][1] ="<a class='pull-right' href=status.php?problem_id=$id&jresult=".$row[0]." >".$row[1]."</a>";
                $i++;
        }
        

//}
$color_list = array('#418CF0', '#FCB441', '#E0400A', '#056492', '#BFBFBF', '#1A3B69', '#FFE382');
?>
...
foreach($result as $row){
        ...
        $view_solution[$j][2]= "<a class='text-primary' style='font-weight:500;font-size:23px;margin: 5px 0px;' href='userinfo.php?user=".$row['user_id']."'>".$row['user_id']." </a><span style='font-weight:500;font-size:23px;margin: 5px 0px;'>  #第".$i."名</span>";
        ...
        if (!(isset($_SESSION[$OJ_NAME.'_'.'user_id'])&&!strcasecmp($row['user_id'],$_SESSION[$OJ_NAME.'_'.'user_id']) ||
                ...
                $view_solution[$j][5]= "<a class='label label-info'>".$language_name[$row['language']]."</a>";
        }else{
                $view_solution[$j][5]=  "<a class='label label-info' target=_blank href=showsource.php?id=".$row['solution_id'].">".$language_name[$row['language']]."</a>";
        }
        if ($flag) $view_solution[$j][6]=  "$s_cl B";
        else $view_solution[$j][6]=  "------";
        $view_solution[$j][7]=  $row['in_date'];
        $view_solution[$j][8]=  $row['user_id'];
        $j++;
